...
% describe the typical characteristics of burst observations along with those 
% properties that can be inferred from simple calculations
...
\end{equation}
% where each of $n$ observartions has exposure $T_i$. 
...
% [{\bf move discussion from later section to here?}]
...
%
% Given an estimate of $Q_{\rm nuc}$, the average hydrogen fraction $\langle X\rangle$ in the fuel layer at ignition has been estimated to date 
In the absence of other information, $\qnuc$ (and hence $y$) has in the past been estimated by adopting a value for 
% the average H-fraction at ignition, 
$\Xb$,
%
...
% average fluence $\langle E_b\rangle$, 
fluences $E_{b,i}$, 
%
...
% (const.c**2*(cd.OPZ-1)/cd.OPZ).to('MeV g-1')*const.m_p.to('g')/(5.22*u.MeV)
% 36.977056, more precisely
...
% \todo{move later sections here on determining the fuel composition?}
...
% (as for analyses based on one burst, as described in \S\ref{subsec:1burst}) 
...
% ... explore a number of methods to infer properties of the bursting source,  
% based on the extent of available observational data, and assess their accuracy.

% seems to be a newer/alternative version of sec:results?
...
% moved the discussion to the results section
% ... and later moved it back --- dkg 9/21

%\subsubsection{The distance to IGR J00291+5934}
%
%Previously we used the absence of thermonuclear bursts in the accretion-powered millisecond pulsar IGR~J00291+5934 to constrain the distance (\S\ref{subsec:zerobursts}).
%%
...
% [{\bf what if you don't want to use the empirical value? }]
...
% moved from later section
% \subsubsection{System properties of IGR~J17591$-$2342}

We use as an example 
% of analysis of a single burst (\S\ref{subsec:1burst}) 
...
% For the single burst observed from IGR~J17591$-$2342 
% Original values; couldn't replicate in 2021 Sep
...
% original text here moved to the earlier "methods" section
\begin{itemize}

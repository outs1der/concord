...
def anisotropy(inclination,model='fuji88',test=False):
...
# want to replicate Figure 2 from fuji88
        ...
        for i, _th in enumerate(_theta):
            xi_b[i], xi_p[i] = anisotropy(_th,model=model)
  
        plt.figure(figsize=(6,10))

        ct = np.cos(_theta)
        plt.plot(ct,1./xi_b,label=r"$\xi_b^{-1}$")
        plt.plot(ct,1./xi_p,label=r"$\xi_p^{-1}$")
        plt.plot(ct,xi_p/xi_b,label=r"$\xi_p/\xi_b$")
        plt.ylim(0,3)
        plt.xlim(0,1)
